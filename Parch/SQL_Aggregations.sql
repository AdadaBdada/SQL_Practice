...
*/
SELECT a.name, AVG(o.standard_qty) avg_stand,
...
*/
SELECT a.name, AVG(o.standard_amt_usd) avg_stand_amt,
AVG(o.gloss_amt_usd) avg_gloss_amt,AVG(o.poster_amt_usd) avg_poster_amt
FROM accounts a
JOIN orders o
ON a.id = o.account_id
GROUP BY a.name
...
SELECT s.id sales_id, s.name sales_name,
...
2013和2017特别小，是因为只包含了一个月的数据，therefore, neither of these are evenly represented
*/
...
*/
  SELECT DATE_PART('year', occurred_at) ord_year,  SUM(total) total_sales
    FROM orders
GROUP BY 1
ORDER BY 2 DESC;
...
4.Which month did Parch & Posey have the greatest sales in terms of total number of orders?
 Are all months evenly represented by the dataset?
 */
 SELECT DATE_PART('month', occurred_at) ord_year,  SUM(total) total_num
   FROM orders
   WHERE occurred_at BETWEEN '2014-01-01' AND '2017-01-01'
GROUP BY 1
ORDER BY 2 DESC;
...
### CASE
/*
1.We would like to understand 3 different levels of customers based on the amount associated
...
/*
3.We would like to identify top performing sales reps,
...
/*
3.The previous didn't account for the middle, nor the dollar amount associated with the sales.
 Management decides they want to see these characteristics represented as well.
 We would like to identify top performing sales reps, which are sales reps associated with more than
 200 orders or more than 750000 in total sales.
